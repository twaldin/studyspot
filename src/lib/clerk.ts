...
export const setSelectedCourseForUser = async (userId: string, courseId: string) => {
  try {
    ...
    logger.info({ userId, courseId }, 'Successfully set selected course for user');
  } catch (error) {
    logger.error({ error, userId, courseId }, 'Error updating user metadata with selected course');
...
export const getSelectedCourseForUser = async (userId: string) => {
  try {
    ...
    logger.error({ error, userId }, 'Error fetching user metadata for selected course');
...
export const getUserOnboardingStatus = async (userId: string) => {
  try {
    ...
    const hasCompletedOnboarding = publicMetadata.hasCompletedOnboarding as boolean;
