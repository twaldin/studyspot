...
export function useDocuments(courseId?: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient<{ docs: Document[] }>(`/docs?courseId=${courseId}`);
...
export function useDocumentsByIds(ids: string[]) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient<{ docs: Document[] }>(`/docs?ids=${ids.join(',')}`);
      return response.docs || [];
    },
...
export function useDocument(documentId?: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient<{ doc: Document }>(`/docs/${documentId}`);
      return response.doc;
    },
