...
const generateEmbeddingsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    if (inputData.skipped || !inputData.isRelevant || inputData.chunks.length === 0) {
      return { embeddings: [], skipped: true };
    }
    
    console.log(`[Embeddings] Generating embeddings for ${inputData.chunks.length} chunks`);
...
export async function executeDocumentIngestion(input: {
  ...
  try {
    ...
    const downloadResult = await downloadAndValidateStep.execute({ 
      inputData: { fileUrl: input.fileUrl, fileName: input.fileName, fileType: input.fileType } 
    });

    // Step 2: Check for duplicates
    onProgress?.('Checking for duplicates...');
    const duplicateResult = await checkDuplicateStep.execute({ 
      inputData: { fileHash: downloadResult.fileHash, courseId: input.courseId } 
    });
